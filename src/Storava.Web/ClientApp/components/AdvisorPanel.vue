...
const props = defineProps<{ session: ScanSession | null }>();
...
async function requestAdvice(): Promise<void> {
  ...
  result.value = null;
  try {
    result.value = await provider.analyze(apiKey.value, settings.value, payload.value);
  } catch (requestError) {
